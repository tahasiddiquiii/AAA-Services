...
public class AAACustomerProtletPortlet extends MVCPortlet {
		    ...
		    public void deleteServices(ActionRequest request, ActionResponse response)
		        throws Exception {
		        long serviceId = ParamUtil.getLong(request, "serviceId");
		        yuvrajServicesLocalServiceUtil.deleteServices(serviceId);
		        sendRedirect(request, response);
		    }
		    ...
		    private yuvrajServices _updateServices(ActionRequest request)
		            throws PortalException, SystemException {
		        ...
		        ServiceContext serviceContext = ServiceContextFactory.getInstance(
		        yuvrajServices.class.getName(), request);
		     
		        yuvrajServices services = null;
		        
		        if (serviceId <= 0) {
		            services = yuvrajServicesLocalServiceUtil.addServices(
		                serviceContext.getUserId(), serviceContext.getScopeGroupId(),
		                serviceName, serviceDescription, servicePrice, serviceContext);
		        }
		        else {
		            services = yuvrajServicesLocalServiceUtil.getyuvrajServices(serviceId);
		            
		            services = yuvrajServicesLocalServiceUtil.updateServices(
		                    serviceContext.getUserId(), serviceId, serviceName, serviceDescription, 
		                    servicePrice, serviceContext);
		        }
		        return services;
		    }
	...
	@Reference
	private yuvrajServicesLocalService _servicesLocalService;
}